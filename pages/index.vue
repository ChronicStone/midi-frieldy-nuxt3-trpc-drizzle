...
const collapsibleRefs = ref<Array<{ setExpanded: (value: boolean) => void; isExpanded: boolean }>>();
...
<template>
  <div class="max-w-screen-xl w-full p-8 mx-auto h-full text-base">
    ...
    <div id="about" class="flex overflow-hidden flex-col-reverse !md:flex-row gap-12 md:h-4/5 items-center">
      <img
        src="/homepage/midi-friendly-group.jpg"
        alt="miudi roupe"
      ...
      <div class="w-full !md:w-1/2 flex flex-col gap-8 mt-20 md:mt-0">
        ...
        <p>
          Le moment le plus important d’une journée de travail n’est pas la réunion, mais bien le déjeuner.
          Echanger, intégrer et rigoler, c’est la période à privilégier pour vous et votre entreprise. Fini la
          prise de tête à demander où manger, MidiFriendly.
        </p>
      </div>
    </div>

    <div id="manage" class="flex overflow-hidden flex-col !md:flex-row gap-12 md:h-4/5 items-center">
      <div class="w-full !md:w-1/2 flex flex-col gap-8 mt-20 md:mt-0">
      ...
      <img
        :src="
          collapsibleSectionTexts?.[activeCollapsibleSection ?? -1]?.image ?? collapsibleSectionTexts[0].image
        "
        alt="miudi roupe"
        class="w-full !md:w-1/2 max-h-60 !md:max-h-4/6 object-cover rounded-md"
      />
