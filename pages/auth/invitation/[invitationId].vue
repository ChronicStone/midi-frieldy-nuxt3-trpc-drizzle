...
import { AuthLoginDto, AuthRegisterDto } from '@/types/auth';
import { AcceptInvitationDto, InvitationPayload } from '@/types/invitation';
...
const route = useRoute('auth-invitation-invitationId');
const messageApi = useMessage();
const { data: invitation } = useRemoteData(() =>
  $client.invitation.validateInvitation.query({
...
function getErrorMessage(
  errorFlags: Pick<InvitationPayload, 'notFound' | 'alreadyUsed' | 'expired' | 'maxUsageReached'>,
) {
  ...
  else if (errorFlags.maxUsageReached) return 'Maximum number of usage for this invitation already reached';
  return 'N/A';
}
...
async function finaliseRegisterProcess(data: AuthRegisterDto | AuthLoginDto) {
  try {
    ...
    const { success, ...errorFlags } = await $client.invitation.acceptInvitationDto.mutate({
      invitationId: route.params.invitationId as string,
      ...(route.query.hash && { emailHash: route.query.hash as string }),
      account: {
    ...
    if (success) {
      ...
      const authData = await $client.auth.login.mutate(data as AuthLoginDto);
